	...
	: radius(rad)
{
	...
	if (urd(gen)) speed = -speed;
}

void Sphere::rotateAxisZ(float theta)
{
	glm::vec3 tmp = axis;
...
void Sphere::draw(GLuint shaderProgram) const
{
...
void Sphere::move(float x, float y, float z, bool changeTargetPos)
{
	pos += glm::vec3(x, y, z);
	mPos += glm::vec3(x, y, z);
	if (changeTargetPos)
